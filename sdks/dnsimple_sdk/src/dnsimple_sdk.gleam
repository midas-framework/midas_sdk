import dnsimple_sdk/operations
...
pub fn change_domain_delegation_to_vanity(token, account, domain, data) {
  let request = base_request(token)
  let request =
...
pub fn change_domain_delegation(token, account, domain, data) {
  let request = base_request(token)
  let request =
    operations.change_domain_delegation_request(request, account, domain, data)
  use response <- t.do(t.fetch(request))
  use data <- t.try(
